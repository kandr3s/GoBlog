...
func (p *plugin) RenderSummaryForPost(rc plugintypes.RenderContext, post plugintypes.Post, doc *goquery.Document) {
	...
	doc.Find(".h-entry").Each(func(i int, s *goquery.Selection) {
		...
		// Bookmarks/Links Post Templates
		...
		if linkSection == "bookmarks" || linkSection == "links" {
			...
			doc.Find(".h-entry p a.u-bookmark-of").Remove()
		}

		// Watches Summaries
		if linkSection == "watches" {
			// Find the last <p> element in .e-content
			lastP := doc.Find(".e-content p").Last()

			// Check if the text of the <p> element starts with "Watched on "
			if strings.HasPrefix(lastP.Text(), "Watched on ") {
				// Remove the <p> element
				lastP.Remove()
			}

			// Add Alt attr to Film Poster
			doc.Find("main.h-entry article div.e-content p img").SetAttr("alt", "Film Poster")
		}

		// Reads Summaries
		// Remove Emoji (already being used by "category")
		if linkSection == "reads" {
			// Find the first <p> element in .e-content
			firstP := doc.Find(".e-content p").First()

			// Remove the emoji
			text := firstP.Text()
			newText := strings.ReplaceAll(text, "📚", "")
			firstP.SetText(newText)
		}

		// Timeline Template
		doc.Find("article").PrependHtml(fmt.Sprintf("<span class=\"category\"><a href=\"/%s\" title=\"%v\">%s</a></span>", post.GetSection(), sectionTitle, sectionEmoji))
		doc.Find("article").AppendHtml(fmt.Sprintf("%s<a href=%s class=\"permalink u-url\">%s</a>", visibility.Text(), permalink, date))
		postfooter.Remove()
		hidden.Remove()
	})
}
			...
			for _, customElements := range customUIElements {
				if element, ok := customElements.(map[string]any); ok {
					...
					if name == "socialicons" {
						...
						// hb.WriteElementClose("svg")
						// hb.WriteElementClose("a")
						hb.WriteElementOpen("a", "class", "social-link", "rel", "nofollow", "target", "_blank", "href", "https://micro.blog/kandr3s", "title", "Micro.blog")
					...
					if name == "indiewebicons" {
						...
						hb.WriteElementClose("a")
						hb.WriteElementOpen("a", "href", "/colophon")
						hb.WriteElementOpen("img", "src", "/images/indieweb-icons.jpg", "title", "Powered by GoBlog & IndieWeb technologies", "alt", "GoBlog, Webmention, Microformats and IndieWeb banners")
						hb.WriteElementClose("a")
